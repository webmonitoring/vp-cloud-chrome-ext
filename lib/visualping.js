const LOCAL_STORAGE_ITEM_SIZE_LIMIT = 2300;
const MAX_ACTIONS_SIZE = 50000;
...
function enforceSizeLimit(value) {
  return JSON.stringify(value).length < LOCAL_STORAGE_ITEM_SIZE_LIMIT;
}

function getLocalStorageScript(values) {
  let script = '';
  Object.keys(values)
    .filter((key) => enforceSizeLimit(values[key]))
    .forEach((key) => {
      script += `localStorage.setItem(${JSON.stringify(key)}, ${JSON.stringify(values[key])});\n`;
    });
  return script;
}

export function mapLocalStorageValuesToPreactions(values) {
  const script = getLocalStorageScript(values);
  if (!script) return [];
  return [
    { script },
    { refresh: true },
...
function removeLargestAction(actions) {
  const largest = actions.reduce(
    (acc, action) => {
      const size = JSON.stringify(action).length;
      return size > acc.size ? { size, action } : acc;
    },
    { size: 0, action: null }
  );
  const index = actions.indexOf(largest.action);
  if (index >= 0) actions.splice(index, 1);
}

export function omitLargeActionsIfNeeded(actions) {
  let actionsSize = JSON.stringify(actions).length;
  while (actionsSize > MAX_ACTIONS_SIZE) {
    removeLargestAction(actions);
    actionsSize = JSON.stringify(actions).length;
  }
}
...
export function buildCreateJobPayload({ url, title, alertCondition, interval, cookies, workspaceId, localStorageActions = [] }) {
  const cookieActions = cookies.length ? buildCookiePreactions(cookies).actions : [];
  const allActions = [...cookieActions, ...localStorageActions];
  omitLargeActionsIfNeeded(allActions);
  ...
  return {
    ...
    preactions: allActions.length ? { active: true, actions: allActions } : undefined,

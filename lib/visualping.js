...
export function buildCreateJobFromSavedSettingsPayload({
  url,
  title,
  workspaceId,
  interval,
...
export function extractSavedJobSettingsList(payload) {
  ...
  const nested =
    payload.savedJobSettings ??
    payload.items ??
    payload.data ??
    payload.results ??
    payload.savedJobSettingList ??
    payload.records ??
    payload.content ??
    payload.elements ??
    payload.savedJobSettingDTOs ??
    payload.savedJobSettingsDtos ??
    payload.settings;
  ...
  const pageContent = payload.page?.content ?? payload.page?.elements;
...
function readSavedJobSettingId(entry) {
  ...
  const pick = (obj) =>
    Number(
      obj.id ??
        obj.savedJobSettingsId ??
        obj.saved_job_settings_id ??
        obj.savedJobSettingId ??
        obj.saved_job_setting_id ??
        obj.presetId ??
        obj.savedJobSettingPresetId
    );
  ...
  const wrappers = [entry.savedJobSetting, entry.savedJobSettingsDTO, entry.savedJobSettingDto, entry.dto, entry.setting, entry.record];
  for (const wrap of wrappers) {
    if (wrap && typeof wrap === "object") {
      const nested = pick(wrap);
...
function readSavedJobSettingName(entry) {
  const nested = entry?.savedJobSetting ?? entry?.savedJobSettingsDTO ?? entry?.savedJobSettingDto ?? entry?.dto ?? entry?.setting ?? {};
  const raw =
    entry?.name ??
    entry?.title ??
    entry?.description ??
    entry?.label ??
    entry?.presetName ??
    entry?.savedJobSettingName ??
    nested?.name ??
    nested?.title ??
    nested?.description ??
    nested?.label ??
    nested?.presetName ??
    nested?.savedJobSettingName ??
    "";
...
function readPresetImportantDefinition(entry) {
  ...
  const summalyzer = settings?.summalyzer && typeof settings.summalyzer === "object" ? settings.summalyzer : {};

  const raw =
    summalyzer.importantDefinition ??
    summalyzer.important_definition ??
    settings.importantDefinition ??
    settings.important_definition ??
    entry?.importantDefinition ??
    entry?.important_definition ??
    "";
...
function readPresetInterval(entry) {
  const settings = entry?.settings && typeof entry.settings === "object" ? entry.settings : {};
  const raw = settings.interval ?? entry?.interval;
...
function isDefaultSavedPreset(entry) {
  ...
  const defaultWorkspaceLists = [entry.defaultForWorkspaces, entry.defaultWorkspaceIds, entry.workspaceDefaults];
  for (const list of defaultWorkspaceLists) {
    if (Array.isArray(list) && list.length > 0) {
      return true;
    }
  }

  if (entry.isDefault === true || entry.default === true || entry.isWorkspaceDefault === true || entry.defaultForWorkspace === true || entry.defaultPreset === true || entry.isDefaultPreset === true) {
    return true;
  }

  const role = String(entry.role ?? entry.presetRole ?? "").toLowerCase();
  return role === "default" || role === "workspace_default";
}
...
function inferPresetWorkspaceIds(entry) {
  ...
  const add = (raw) => {
    ...
    if (Number.isFinite(numeric) && numeric !== 0) {
      ids.add(Math.abs(Math.trunc(numeric)));
    }
  };

  const lists = [entry.defaultForWorkspaces, entry.defaultWorkspaceIds, entry.workspaceIds, entry.workspaces];
  for (const list of lists) {
    if (Array.isArray(list)) {
      list.forEach((item) => {
        if (item && typeof item === "object") {
          add(item.id ?? item.workspaceId ?? item.workspace_id);
        } else {
          add(item);
        }
      });
...
export async function listSavedJobSettings(config, token, params = {}) {
  ...
  console.info("[VP Cloud] GET saved-job-settings →", urlString);
  ...
  const data = await response.json();
  const rows = extractSavedJobSettingsList(data).length;
  console.info("[VP Cloud] GET saved-job-settings OK:", response.status, `rows=${rows}`);
  return data;
}

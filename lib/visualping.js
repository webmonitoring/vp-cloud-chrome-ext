...
export async function getVisualpingIdToken(config) {
  ...
  for (const url of cookieUrls) {
    try {
      ...
      console.warn(`Unable to read Visualping idToken cookie for ${url}.`, error);
    }
  }
...
export function buildCreateJobPayload({ url, title, alertCondition, interval, cookies }) {
  return {
    url,
...
export function buildCookieSyncPayload(jobId, cookies) {
  return {
    jobId: Number(jobId),
    enable_cookies_and_ad_blocker: true,
    preactions: buildCookiePreactions(cookies),
  };
}
